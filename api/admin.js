...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (path === '/setup') {
      ...
      const adminUser = users.find(u => u.email === 'alimuyisa6@gmail.com');

      if (!adminUser) {
        const { data: newUser, error: createError } = await supabaseAdmin.auth.admin.createUser({
          email: 'alimuyisa6@gmail.com',
          password: 'Temporary123!',
          email_confirm: true
        });
        if (createError) return res.status(500).json({ error: 'Failed to create user: ' + createError.message });

        await supabase.from('admin_master').insert({
          admin_id: newUser.user.id,
          admin_email: 'alimuyisa6@gmail.com',
        ...
        return res.status(200).json({
          success: true,
          message: 'Admin account created. Password: Temporary123! Change it after login.'
        });
      }
      ...
      return res.status(200).json({
        success: true,
        message: 'Admin setup complete! You can now sign in and access admin.html'
      });
    }
    ...
    // PUBLIC ROUTES
    // ═══════════════════════════════════════
    if (path === '/test') return res.status(200).json({ ok: true });
    ...
    if (path.startsWith('/resources/') && method === 'GET') {
      ...
      if (!r) {
        const { data: s } = await supabase.from('resource_submissions').select('*').eq('id', id).single();
        r = s;
      }
    ...
    if (path.startsWith('/resources/') && method === 'PUT') {
      ...
      if (error) {
        const { error: e2 } = await supabase.from('resource_submissions').update(body).eq('id', id);
        if (e2) throw e2;
      }
    ...
    if (path.startsWith('/resources/') && method === 'DELETE') {
      ...
      if (error) {
        const { error: e2 } = await supabase.from('resource_submissions').delete().eq('id', id);
        if (e2) throw e2;
      }
      return res.status(200).json({ success: true });
    }
    ...
    if (path.startsWith('/site-sections/') && method === 'PUT') {
      ...
      if (existing) {
        await supabase.from('site_sections').update({ data: body }).eq('section', section);
      } else {
        await supabase.from('site_sections').insert({ section, data: body });
      }
      return res.status(200).json({ success: true });
    }
