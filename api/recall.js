...
async function handleGetSession(userId, params) {
  ...
  // FIXED: select without alias, map manually
  ...
  if (!questions || questions.length === 0) throw new Error('No questions available for this level');
...
async function handleSubmitAnswer(userId, params) {
  ...
  // FIXED: remove .catch(()=>{}) – the insert does not return a promise that needs .catch; use try/catch if needed
  try {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Send appropriate status code for "already answered" to avoid frontend treating it as offline
    const status = error.message === 'Question already answered' ? 400 : 500;
