...
module.exports = async (req, res) => {
  ...
  // --- NEW: Support action query parameter (used by the HTML) ---
  if (actionParam) {
    switch (actionParam) {
      case 'stats':
      ...
      case 'answer':
        ...
        if (method === 'POST') {
          ...
          return res.status(405).json({ error: 'Method not allowed. Use POST for answer.' });
        }
        break;
      case 'complete':
        ...
        if (method === 'POST') {
          ...
          return res.status(405).json({ error: 'Method not allowed. Use POST for complete.' });
        }
        break;
      case 'test':
        return res.status(200).json({ message: 'Recall API is reachable', timestamp: Date.now() });
      default:
