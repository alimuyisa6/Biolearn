...
// FIX: New check using topic_completion
async function handleCheckSession(userId, params) {
  const { level, topic } = params;
  const today = new Date().toISOString().split('T')[0];
  const topicKey = topic || 'all'; // 'all' for sessions without a specific topic
  // Get user's topic_completion map
  ...
  const lastCompletedDate = completion[topicKey];
...
async function handleGetSession(userId, params) {
  ...
  // Check if already completed for this topic (by looking at topic_completion)
...
async function handleCompleteSession(userId, params) {
  ...
  // FIX: Update topic_completion in user_recall_stats
  const today = new Date().toISOString().split('T')[0];
