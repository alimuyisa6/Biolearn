...
async function handleGetSession(userId, params) {
  ...
  let completedQuery = supabase
    ...
    .gte('created_at', today)
    .eq('is_active', false);
  if (topic) completedQuery = completedQuery.eq('topic', topic);
  else completedQuery = completedQuery.is('topic', null);
...
async function handleSubmitAnswer(userId, params) {
  ...
  if (session.current_index >= session.questions.length) throw new Error('All questions already answered');
...
async function handleCompleteSession(userId, params) {
  const { session_id } = params;
  const { data: session, error: sessionError } = await supabase.from('recall_sessions').select('user_answers').eq('session_id', session_id).eq('user_id', userId).single();
  ...
  const { data: stats, error: statsError } = await supabase.from('user_recall_stats').select('total_sessions, current_streak').eq('user_id', userId).single();
  if (statsError && statsError.code !== 'PGRST116') throw new Error(`Stats fetch error: ${statsError.message}`);
  if (stats) {
    const { error: incError } = await supabase.from('user_recall_stats').update({ total_sessions: (stats.total_sessions || 0) + 1 }).eq('user_id', userId);
