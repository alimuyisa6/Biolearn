...
async function handleContinueSession(userId, { session_id }) {
  ...
  if (nextBatch.length === 0) {
    return { has_more: false, questions: [] };
  }
  ...
  if (action) {
    switch (action) {
      ...
      case 'continue':
        ...
        let continueBody = '';
        await new Promise((resolve) => { req.on('data', chunk => continueBody += chunk); req.on('end', resolve); });
        try { params = JSON.parse(continueBody); } catch (e) { return res.status(400).json({ error: 'Invalid JSON body' }); }
