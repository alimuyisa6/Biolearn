...
function containsExactWord(sentence, word) {
  const words = wordSplit(sentence);
  return words.includes(word.toLowerCase());
}

function containsExactPhrase(sentence, phrase) {
  const normSentence = sentence.toLowerCase().trim();
  const normPhrase = phrase.toLowerCase().trim();
  return normSentence.includes(normPhrase);
}
...
function calculateRecallStrength(userAnswer, correctAnswer, alternateAnswers = []) {
  ...
  for (const concept of allCorrect) {
    ...
    const isSingleWord = !normalizedConcept.includes(' ');
    if (isSingleWord) {
      if (containsExactWord(normalizedAnswer, normalizedConcept)) {
        return { strength: 'excellent', matched: concept, xp: 10, needNote: false };
      }
    } else {
      if (containsExactPhrase(normalizedAnswer, normalizedConcept)) {
        return { strength: 'excellent', matched: concept, xp: 10, needNote: false };
      }
    }
  }
