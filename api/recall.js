...
async function getUserFromSession(req) {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  console.log("SESSION_LOOKUP_DEBUG", {
    hashedToken,
    found: !!data,
  ...
  if (error || !data) {
    console.log("AUTH_FAILURE_DEBUG", {
...
async function isSuperAdmin(userId) {
 ...
 function sendJson(res, statusCode, data, csrfSecret) {
  if (!csrfSecret) {
    res.status(statusCode).json({ data, csrf_token: null });
    return;
  }
  const windowedTimestamp = Math.floor(Date.now() / (15 * 60 * 1000)) * (15 * 60 * 1000);
  const hmac = crypto.createHmac('sha256', csrfSecret).update(windowedTimestamp.toString()).digest('hex');
  const csrfToken = `${windowedTimestamp}.${hmac}`;
  res.status(statusCode).json({ data, csrf_token: csrfToken });
}
...
module.exports = async (req, res) => {
      ...
      case 'topics':
        {
          ...
          console.log("RESPONSE_DEBUG", { handler: 'topics', userId: user.user_id, success: true });
          return sendJson(res, 200, topics, user.csrf_secret);
        }
      case 'first_visit':
        {
          const level = url.searchParams.get('level');
          ...
          console.log("RESPONSE_DEBUG", { handler: 'first_visit', userId: user.user_id, success: true });
          return sendJson(res, 200, result, user.csrf_secret);
        }
      case 'get_selected_level':
        {
          ...
          const result = { selected_level: data?.selected_level || null, is_super_admin: isAdmin };
          console.log("RESPONSE_DEBUG", { handler: 'get_selected_level', userId: user.user_id, success: true });
          return sendJson(res, 200, result, user.csrf_secret);
        }
      case 'set_selected_level':
        {
          try {
            ...
            console.log("RESPONSE_DEBUG", { handler: 'set_selected_level', userId: user.user_id, success: true });
            return sendJson(res, 200, { success: true }, user.csrf_secret);
          } catch (csrfError) {
            return res.status(403).json({ error: 'Invalid or missing CSRF token' });
          }
        }
  ...
  try {
    ...
    console.log("RESPONSE_DEBUG", { handler: handler.name, userId: user.user_id, success: true });
    sendJson(res, 200, result, user.csrf_secret);
  } catch (error) {
