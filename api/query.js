  ...
  try {
    ...
    switch (action) {
      ...
      case 'toggle_flashcard_known': {
        ...
        const { data: existing } = await supabase.from('user_interactions').select('id').eq('user_id', userId).eq('interaction_type', 'flashcard_known').eq('metadata->>flashcard_id', String(fid)).maybeSingle();
        if (existing) { await supabase.from('user_interactions').delete().eq('id', existing.id); result = { known: false }; }
        else { await supabase.from('user_interactions').insert({ user_id: userId, interaction_type: 'flashcard_known', resource_id: null, metadata: { flashcard_id: fid } }); result = { known: true }; }
        break;
      }
      ...
      case 'rate_flashcard': {
        ...
        const { data: existing } = await supabase.from('user_interactions').select('id').eq('user_id', userId).eq('resource_id', fid).eq('interaction_type', 'rating').eq('metadata->>type', 'flashcard_difficulty').maybeSingle();
      ...
      case 'toggle_flashcard_bookmark': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        const fid = req.body.flashcard_id || flashcard_id;
        const { data: existing } = await supabase.from('user_interactions').select('id').eq('user_id', userId).eq('resource_id', fid).eq('interaction_type', 'favorite').eq('metadata->>type', 'flashcard_bookmark').maybeSingle();
        if (existing) { await supabase.from('user_interactions').delete().eq('id', existing.id); result = { bookmarked: false }; }
        else { await supabase.from('user_interactions').insert({ user_id: userId, interaction_type: 'favorite', resource_id: fid, metadata: { type: 'flashcard_bookmark' } }); result = { bookmarked: true }; }
        break;
      }
      case 'like_resource': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        const rid = req.body.resource_id || resource_id;
        const { data: existing } = await supabase.from('user_interactions').select('id').eq('user_id', userId).eq('resource_id', rid).eq('interaction_type', 'favorite').eq('metadata->>type', 'like').maybeSingle();
        if (existing) { await supabase.from('user_interactions').delete().eq('id', existing.id); }
        else { await supabase.from('user_interactions').insert({ user_id: userId, interaction_type: 'favorite', resource_id: rid, metadata: { type: 'like' } }); }
        const { count } = await supabase.from('user_interactions').select('id', { count: 'exact', head: true }).eq('resource_id', rid).eq('interaction_type', 'favorite').eq('metadata->>type', 'like');
      ...
      case 'get_resource_interactions': {
        const rid = req.body.resource_id || resource_id;
        const { count: likeCount } = await supabase.from('user_interactions').select('id', { count: 'exact', head: true }).eq('resource_id', rid).eq('interaction_type', 'favorite').eq('metadata->>type', 'like');
      ...
      case 'submit_weekly_challenge': {
        ...
        const { data: existing } = await supabase.from('user_interactions').select('metadata').eq('user_id', userId).eq('interaction_type', 'weekly_challenge_answer').eq('metadata->>week_start', ws).maybeSingle();
        if (so === null || so === undefined) {
          ...
          await supabase.from('user_interactions').insert({ user_id: userId, interaction_type: 'weekly_challenge_answer', resource_id: null, metadata: { week_start: ws, selected_option: so, is_correct: isCorrect } });
