    ...
    switch (action) {
      ...
      case 'submit_weekly_challenge': {
        ...
        if (selected_option === null || selected_option === undefined) {
          ...
          if (existing) {
            return res.status(409).json({ error: 'Already answered this week' });
          }
          const { data: wcData } = await supabase
            .from('site_sections')
            .select('data')
          ...
          await supabase.from('user_interactions').insert({
            ...
            metadata: { selected_option, is_correct: isCorrect }
          });
          result = {
            already_answered: true,
            selected_option,
