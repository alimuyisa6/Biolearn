...
const CSRF_PROTECTED_ACTIONS = new Set([
  ...
  'submit_quiz_answers', 'add_quiz_questions_batch'
]);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'check_daily_retry': {
        ...
        const { level: rl, topic: rt } = req.body;
        const { data, error } = await supabase.from('user_quiz_activity').select('completed_at,passed').eq('user_id',userId).eq('level',rl).eq('topic',rt).order('completed_at',{ascending:false}).limit(1);
        if (error) throw error;
