    ...
    switch (action) {
      ...
      case 'get_app_features': {
        ...
        if (page_id === 'quiz' && userId) {
          const { data: userSettings, error: usError } = await supabase.from('user_feature_settings').select('feature_key, is_enabled, custom_settings').eq('user_id', userId);
          if (usError) { result = data.map(f => ({ ...f, user_enabled: f.is_enabled, user_settings: {} })); break; }
          const userFeatureMap = new Map(); (userSettings || []).forEach(s => userFeatureMap.set(s.feature_key, s));
          result = data.map(feature => { const override = userFeatureMap.get(feature.feature_key); return { ...feature, user_enabled: override ? override.is_enabled : feature.is_enabled, user_settings: override ? (override.custom_settings || {}) : {} }; });
        } else { result = data.map(f => ({ ...f, user_enabled: f.is_enabled, user_settings: {} })); }
        break;
      }
      case 'toggle_feature': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
