  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_all_site_sections': case 'get_all_sections': {
        ...
        // Include users list for admin dashboards
        if (adminData) {
          try {
            ...
            result.users = {
              list: (users?.users || []).map(u => ({
                ...
                is_locked: adminMap.get(u.id)?.is_locked || false,
      ...
      case 'check_quiz_answer': {
        ...
        const { data: question, error } = await supabase.from('quiz_questions').select('id,correct_option,explanation,option_a,option_b,option_c,option_d').eq('id',question_id).single();
        ...
        result = { correct: isCorrect, correct_option: question.correct_option, correct_answer_text: optionMap[question.correct_option], explanation: question.explanation };
        break;
      }
      ...
      case 'get_user': {
        ...
        result={
          user: {
            ...
            is_locked: adminData?.is_locked || false
          }
        };
        break;
      }
      ...
      case 'approve': { if(!submissionId||!['approve','reject','delete'].includes(req.body.action))throw new Error('Invalid approval action');
        if (req.body.action === 'delete') {
          ...
          await supabase.from('resource_submissions').update({status:req.body.action==='approve'?'approved':'rejected'}).eq('id',submissionId);
        }
      ...
      case 'update_site_section': {
        ...
        if (existing) {
          await supabase.from('site_sections').update({ data: secData }).eq('section', sec);
        } else {
          await supabase.from('site_sections').insert({ section: sec, data: secData });
        }
        ...
        if (role === 'admin') {
          ...
          if (!existingAdmin) {
            await supabase.from('admin_master').insert({
              ...
              admin_email: (await supabase.auth.admin.getUserById(targetUserId)).data?.user?.email || '',
              admin_role: 'content_manager',
              permissions: { can_manage_resources: true, can_manage_site_sections: true, can_view_analytics: true, can_upload_files: true }
            });
          } else {
            await supabase.from('admin_master').update({ admin_role: 'content_manager', is_active: true }).eq('admin_id', targetUserId);
          }
        }
        result = { success: true };
        break;
