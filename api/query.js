...
const PUBLIC_ACTIONS = new Set([
  ...
  'get_all_ratings',
  'check_admin_online'   // public presence check
]);
...
const ADMIN_ACTIONS = new Set([
  ...
  // Chat admin actions
  'admin_get_pending_requests', 'admin_accept_chat', 'admin_reject_chat',
  'admin_update_presence', 'admin_get_active_chats'
]);
...
async function handlePost(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'toggle_favorite': {
        ...
        const { data: existing } = await supabase.from('user_interactions')
          .select('id').eq('user_id', userId).eq('resource_id', resource_id)
          .eq('interaction_type', 'favorite').maybeSingle();
        if (existing) {
          await supabase.from('user_interactions').delete().eq('id', existing.id);
          result = { favorited: false };
        } else {
          await supabase.from('user_interactions').insert({
            user_id: userId, resource_id, interaction_type: 'favorite'
          });
      ...
      case 'record_view': {
        ...
        const { resource_id } = req.body;
        await supabase.from('user_interactions').insert({
          user_id: userId, resource_id, interaction_type: 'view'
        });
        result = { success: true };
        break;
      }
      ...
      case 'record_download': {
        ...
        const { resource_id } = req.body;
        await supabase.from('user_interactions').insert({
          user_id: userId, resource_id, interaction_type: 'download'
        });
        result = { success: true };
        break;
      }
      ...
      case 'record_daily_visit': {
        ...
        const { data: existing } = await supabase.from('user_interactions')
          .select('id').eq('user_id', userId).eq('interaction_type', 'daily_visit')
          .gte('created_at', `${today}T00:00:00Z`).lte('created_at', `${today}T23:59:59Z`)
          .limit(1);
        if (!existing || existing.length === 0) {
          await supabase.from('user_interactions').insert({
            user_id: userId, interaction_type: 'daily_visit'
          });
        }
        result = { success: true };
        break;
      ...
      case 'get_user_favorites': {
        ...
        const { data, error } = await supabase.from('user_interactions')
          .select('resource_id, resources:biology_notes(title)')
          .eq('user_id', userId).eq('interaction_type', 'favorite')
          .order('created_at', { ascending: false });
        if (error) throw error;
        result = (data || []).map(f => ({
          resource_id: f.resource_id,
          title: f.resources?.title || null
        }));
        break;
      }
      ...
      case 'get_recent_views': {
        ...
        const { data, error } = await supabase.from('user_interactions')
          .select('resource_id, created_at, resources:biology_notes(title)')
          .eq('user_id', userId).eq('interaction_type', 'view')
          .order('created_at', { ascending: false }).limit(limit);
        if (error) throw error;
        result = (data || []).map(v => ({
          resource_id: v.resource_id,
          title: v.resources?.title || null,
      ...
      case 'submit_rating': {
        ...
        if (existing) {
          const { error } = await supabase
            .from('user_interactions')
            .update({ value: rating, metadata: metadata || {}, created_at: new Date().toISOString() })
            .eq('id', existing.id);
          if (error) throw error;
        } else {
          const { error } = await supabase
            .from('user_interactions')
            .insert({ user_id: userId, resource_id, interaction_type: 'rating', value: rating, metadata: metadata || {} });
          if (error) throw error;
        }
        result = { success: true };
        break;
      ...
      case 'get_all_ratings': {
        ...
        const resultObj = {};
        for (const [rid, { sum, count }] of Object.entries(agg)) {
          resultObj[rid] = { avg: +(sum / count).toFixed(1), count };
        }
        result = resultObj;
        break;
      }
      ...
      case 'get_user_ratings': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        const { data, error } = await supabase.from('user_interactions')
          .select('resource_id, value').eq('user_id', userId).eq('interaction_type', 'rating');
        if (error) throw error;
        const userRatings = {};
      ...
      case 'get_user_achievements': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        const { data, error } = await supabase.from('user_interactions')
          .select('metadata').eq('user_id', userId).eq('interaction_type', 'achievement');
        if (error) throw error;
      ...
      case 'get_user_streak': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        const { data, error } = await supabase.from('user_interactions')
          .select('created_at').eq('user_id', userId).eq('interaction_type', 'daily_visit')
          .order('created_at', { ascending: false });
        if (error) throw error;
      ...
      case 'request_chat': {
        ...
        const { data: existingRoom } = await supabase
          ...
          .maybeSingle();
        if (existingRoom) {
          result = { room_id: existingRoom.id, status: existingRoom.status };
          break;
        }
        ...
        const { data: freeAdmin } = await supabase
          .from('admin_master')
          .select('admin_id')
          .eq('is_online', true)
          .eq('is_busy', false)
          ...
          .maybeSingle();
        if (freeAdmin) {
          await supabase.from('chat_rooms').update({ assigned_admin: freeAdmin.admin_id, status: 'active' }).eq('id', room.id);
          await supabase.from('admin_master').update({ is_busy: true, current_room: room.id }).eq('admin_id', freeAdmin.admin_id);
          room.status = 'active';
        }
      ...
      case 'get_chat_messages': {
        ...
        if (!adminData) {
          const { data: room } = await supabase.from('chat_rooms').select('user_id').eq('id', room_id).single();
      ...
      case 'send_chat_message': {
        ...
        if (!adminData) {
          ...
          const { data: room } = await supabase.from('chat_rooms').select('assigned_admin').eq('id', room_id).single();
          if (!room || room.assigned_admin !== userId) return res.status(403).json({ error: 'Not your conversation' });
        }
        const senderType = adminData ? 'admin' : 'user';
        const { data: msg, error } = await supabase.from('chat_messages').insert({
          room_id, sender_type: senderType, content: sanitizeInput(message, 2000)
        }).select().single();
        if (error) throw error;
        result = msg;
        break;
      ...
      case 'admin_get_pending_requests': {
        ...
        for (const r of (data || [])) {
          try {
            ...
            requests.push({ ...r, user_name: userName });
          } catch { requests.push({ ...r, user_name: 'User' }); }
        }
        result = requests;
        break;
      }
      ...
      case 'admin_get_active_chats': {
        ...
        for (const r of (data || [])) {
          try {
            const { data: { user } } = await supabase.auth.admin.getUserById(r.user_id);
            const userName = user?.email ? user.email.split('@')[0].replace(/[._]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'User';
            chats.push({ ...r, user_name: userName });
  ...
  try {
    ...
    const { data, error } = await supabase.storage
      .from('resources')
      .upload(path, buf, { contentType: `application/${ext}` });
    if (error) throw error;
