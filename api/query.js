...
function rateLimit(ip, action = null) {
  ...
  if (record.count > maxAllowed * 3) {
    BANNED_IPS.add(ip);
    BANNED_UNTIL.set(ip, now + 10 * 60 * 1000);
    setTimeout(() => {
...
setInterval(() => {
  ...
  for (const [key, attempts] of FAILED_ATTEMPTS) {
    if (now - attempts.firstAttempt > 3600000) FAILED_ATTEMPTS.delete(key);
  }
}, 60000);
...
module.exports = async (req, res) => {
  ...
  if (!rateLimit(ip)) {
    logSecurityEvent('RATE_LIMIT_EXCEEDED', { method: req.method }, req);
...
async function handleGet(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_quizzes': {
        const category = url.searchParams.get('category');
        let query = supabase.from('quizzes').select('id,title,category,description,total_points,difficulty,is_active').eq('is_active', true);
