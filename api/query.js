...
const PUBLIC_ACTIONS = new Set([
  ...
  // new public read actions
  'get_all_ratings'   // aggregated ratings are public
]);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_user_streak': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        // Get all daily_visit dates for user, order by date desc
        const { data, error } = await supabase.from('user_interactions')
        ...
        // Check today first; if not visited today, check yesterday as start
