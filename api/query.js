    ...
    switch (action) {
      ...
      case 'get_quiz_topics': {
        ...
        if (topics.length > 0) {
          ...
          topics.forEach(t => {
            t.question_count = countMap.get(t.topic_name) || 0;
            t.total_blocks = Math.ceil(t.question_count / 10);
          });
          if (userId) {
            ...
            (blocks || []).forEach(b => {
              ...
              if (!latestAttempts.has(key) || new Date(b.completed_at) > new Date(latestAttempts.get(key).completed_at)) {
                latestAttempts.set(key, b);
              }
            });
            topics.forEach(t => {
              const completed = [];
              ...
              for (let i = 0; i < t.total_blocks; i++) {
                ...
                const attempt = latestAttempts.get(key);
                if (attempt) {
                  completed.push(i);
                  const attemptDate = new Date(attempt.completed_at).toISOString().slice(0,10);
                  if (attemptDate === todayStr) locked.push(i);
                }
              }
              t.completed_blocks = completed;
              t.locked_blocks = locked;
            });
          }
        }
        result = topics;
      ...
      case 'submit_quiz_block': {
        ...
        if (existingAttempt) {
          return res.status(429).json({ error: 'You have already attempted this block today. Please try again tomorrow.' });
        }
        ...
        if (achievements && Array.isArray(achievements)) {
          const achievementInserts = achievements.map(badge => ({
            user_id: userId,
            interaction_type: 'achievement',
            metadata: { badge: badge.id || badge, ...badge }
          }));
      ...
      case 'toggle_flashcard_known': {
        ...
        const { data: existing } = await supabase
          ...
          .select('id')
          .eq('user_id', userId)
          .eq('interaction_type', 'flashcard_known')
          .eq('metadata->>flashcard_id', String(flashcard_id))
          .maybeSingle();
        if (existing) {
          ...
          await supabase.from('user_interactions').insert({
            user_id: userId,
            interaction_type: 'flashcard_known',
            resource_id: null,
            metadata: { flashcard_id: flashcard_id }
          });
          result = { known: true };
        }
        break;
      ...
      case 'get_community_activity': {
        const { data: downloads } = await supabase
          .from('user_interactions')
          .select('created_at, resource_id, resources:biology_notes(title)')
        ...
        const { data: quizCompletions } = await supabase
          ...
          .select('completed_at, level, topic, percentage')
          .eq('passed', true)
        ...
        (downloads || []).forEach(d => {
          const title = d.resources?.title || 'a resource';
          activities.push({
            type: 'download',
            message: `A learner downloaded "${title}"`,
            time: d.created_at
          });
        });
        ...
        (quizCompletions || []).forEach(q => {
          activities.push({
            type: 'quiz',
            message: `A learner passed a ${q.level} quiz on ${q.topic} with ${q.percentage}%`,
            time: q.completed_at
          });
        });
      ...
      case 'submit_weekly_challenge': {
        ...
        const { data: existing } = await supabase
          ...
          .select('metadata')
          .eq('user_id', userId)
          .eq('interaction_type', 'weekly_challenge_answer')
          .eq('metadata->>week_start', week_start)
          .maybeSingle();
        if (selected_option === null || selected_option === undefined) {
          if (existing) {
            const metadata = existing.metadata || {};
            const { data: wcData } = await supabase
              .from('site_sections').select('data')
              .eq('section', 'weekly_challenge').single();
          ...
          await supabase.from('user_interactions').insert({
            user_id: userId,
            interaction_type: 'weekly_challenge_answer',
            resource_id: null,
            metadata: { week_start, selected_option, is_correct: isCorrect }
          });
          result = {
            already_answered: true,
            selected_option,
