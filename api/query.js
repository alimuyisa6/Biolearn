    ...
    switch (action) {
      ...
      case 'get_app_features': {
        ...
        if (page_id === 'quiz' && userId) {
          ...
          result = (data || []).map(f => ({ ...f, user_enabled: userMap.has(f.feature_key) ? userMap.get(f.feature_key).is_enabled : true, user_settings: userMap.has(f.feature_key) ? userMap.get(f.feature_key).custom_settings : {} }));
        } else {
          result = data || [];
        }
        break;
      ...
      case 'get_user_stats': {
        if (!userId) return res.status(401).json({ error: 'Authentication required' });
        const { data, error } = await supabase.from('user_quiz_activity').select('*').eq('user_id', userId).order('completed_at', { ascending: false });
        if (error) throw error;
        const all = data || [];
        const passed = all.filter(a => a.passed);
        const uniqueTopics = [...new Set(all.map(a => a.topic))];
        result = {
          total_blocks_attempted: all.length,
          total_blocks_passed: passed.length,
          average_score: all.length ? Math.round(all.reduce((s, a) => s + a.percentage, 0) / all.length) : 0,
          best_score: all.length ? Math.max(...all.map(a => a.percentage)) : 0,
          topics_attempted: uniqueTopics.length,
          streak: calculateStreak(all),
          recent_activity: all.slice(0, 10)
        };
        break;
      }
      ...
      case 'signup': {
        ...
        let session = null;
        if (data.session) { const cs = await createUserSession(data.user.id, data.user.email, ip, req.headers['user-agent']); session = cs; }
      ...
      case 'get_user': {
        ...
        result={ user: { id: userId, email: userEmail, is_admin: !!adminData, admin_role: adminData?.admin_role || null, permissions: adminData?.permissions || null, is_locked: adminData?.is_locked || false } };
        break;
      }
      ...
      case 'approve': { if(!submissionId||!['approve','reject','delete'].includes(req.body.action))throw new Error('Invalid approval action');
        if (req.body.action === 'delete') { const { data: sub } = await supabase.from('resource_submissions').select('id').eq('id', submissionId).maybeSingle(); if (sub) { await supabase.from('resource_submissions').delete().eq('id', submissionId); } else { await supabase.from('biology_notes').delete().eq('id', submissionId); } }
        else { await supabase.from('resource_submissions').update({status:req.body.action==='approve'?'approved':'rejected'}).eq('id',submissionId); }
      ...
      case 'update_site_section': {
        const { section: sec, data: secData } = req.body;
        const { data: existing } = await supabase.from('site_sections').select('id').eq('section', sec).maybeSingle();
        if (existing) { await supabase.from('site_sections').update({ data: secData }).eq('section', sec); }
        else { await supabase.from('site_sections').insert({ section: sec, data: secData }); }
        responseCache.delete('all_sections'); result = { success: true };
        break;
      }
      case 'update_user_role': {
        ...
        if (role === 'admin') {
          ...
          if (!existingAdmin) { await supabase.from('admin_master').insert({ admin_id: targetUserId, admin_email: (await supabase.auth.admin.getUserById(targetUserId)).data?.user?.email || '', admin_role: 'content_manager', permissions: { can_manage_resources: true, can_manage_site_sections: true, can_view_analytics: true, can_upload_files: true } }); }
          else { await supabase.from('admin_master').update({ admin_role: 'content_manager', is_active: true }).eq('admin_id', targetUserId); }
        }
        result = { success: true };
        break;
      }
      case 'update_user_lock': {
        const { userId: targetUserId, lock, reason } = req.body;
        await supabase.from('admin_master').update({ is_locked: lock, lock_reason: reason || null }).eq('admin_id', targetUserId);
        result = { success: true };
        break;
      }
