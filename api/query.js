...
setInterval(() => {
  const now = Date.now();
  for (const [ip, record] of RATE_LIMITS) {
    if (now > record.reset) RATE_LIMITS.delete(ip);
  }
  for (const [key, attempts] of AUTH_ATTEMPTS) {
    if (now - attempts.firstAttempt > 3600000) AUTH_ATTEMPTS.delete(key);
  }
}, 60000);
...
module.exports = async (req, res) => {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, X-CSRF-Token, X-Request-ID, X-Turnstile-Token');
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_quizzes': {
        ...
        if (userId && data && data.length) {
          ...
          const { data: progress } = await supabase
            ...
            .select('quiz_id, score, total_possible, percentage, passed, completed_at')
            .eq('user_id', userId)
            .in('quiz_id', quizIds);
      ...
      case 'get_user_progress': {
        ...
        const { data, error } = await supabase
          .from('user_quiz_activity')
          .select('id,quiz_id,score,total_possible,percentage,passed,completed_at,answers,time_taken')
          .eq('user_id', userId)
...
async function handlePost(req, res) {
  ...
  if (CSRF_PROTECTED_ACTIONS.has(action)) {
    ...
    if (clientToken && process.env.TURNSTILE_SECRET_KEY) {
      const valid = await verifyTurnstile(clientToken, ip);
      if (!valid) {
        logSecurityEvent('TURNSTILE_FAILED', { action }, req);
      }
    }
  }
    ...
    switch (action) {
      ...
      case 'signup': {
        if (BANNED_IPS.has(ip)) {
          ...
          if (banUntil && banUntil > Date.now()) {
            const minutes = Math.ceil((banUntil - Date.now()) / 60000);
            return res.status(429).json({ error: `Too many attempts. Please wait ${minutes} minute(s) and try again.` });
          }
          BANNED_IPS.delete(ip);
          BANNED_UNTIL.delete(ip);
        }
        ...
        if (error) {
          if (error.code === 'user_already_exists' || error.message?.includes('already')) {
            return res.status(200).json({ data: { user: null, session: null, message: 'If this account exists, check your email to sign in.' } });
          }
      ...
      case 'signin': {
        if (BANNED_IPS.has(ip)) {
          ...
          if (banUntil && banUntil > Date.now()) {
            const minutes = Math.ceil((banUntil - Date.now()) / 60000);
            return res.status(429).json({ error: `Account temporarily locked. Please wait ${minutes} minute(s) and try again.` });
          }
          BANNED_IPS.delete(ip);
          BANNED_UNTIL.delete(ip);
        }
        ...
        if (error) {
          ...
          if (banned) {
            return res.status(429).json({ error: 'Too many failed attempts. Account temporarily locked for 15 minutes.' });
          }
          throw error;
        }
        resetFailedAuth(ip, email);
      ...
      case 'complete_quiz': {
        if (!userId) {
          return res.status(401).json({ error: 'Please sign in to save your quiz results.' });
        }
      ...
      case 'add_reaction': {
        if (!userId) {
          return res.status(401).json({ error: 'Please sign in to react.' });
        }
